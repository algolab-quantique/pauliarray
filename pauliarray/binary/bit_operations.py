...
def row_echelon_with_map(bit_matrix: "np.ndarray[np.bool]") -> "np.ndarray[np.bool]":
    """
    Applies Gauss-Jordan elimination on a binary matrix to produce row echelon form.
    ...
    map_matrix = np.identity(bit_matrix.shape[0])
...
def row_space_with_map(bits: "np.ndarray[np.bool]") -> "np.ndarray[np.bool]":
    """
    Computes the row space of a binary matrix using Gauss-Jordan elimination and
    removing the zero lines.
    ...
    return row_ech_bits[~null_rows, :], inv(inv_map_matrix)[:, ~null_rows]
